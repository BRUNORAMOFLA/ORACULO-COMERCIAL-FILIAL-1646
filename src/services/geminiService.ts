...
- Tríplice Coroa: ${
    Object.values(data.store.tripleCrownStatus).every((v) => v)
      ? "Consolidada"
      : "Pendente"
  }
- Execução Operacional (Cartões/Combos): ${(
    (data.store.pillars.operational.cards.achievement +
      data.store.pillars.operational.combos.achievement) /
    2
  ).toFixed(1)}%
- Projeção Mensal: Mercantil ${data.projection.mercantilProjected.toFixed(
    1
  )}%, CDC ${data.projection.cdcProjected.toFixed(
    1
  )}%, Serviços ${data.projection.servicesProjected.toFixed(1)}%
- Dependência: ${data.distribution.dependencyLevel} (Concentração Top 1: ${data.distribution.top1Contribution.toFixed(
    1
  )}%)
- Maturidade do Time: ${data.maturityIndex.classification} (${data.maturityIndex.above100Percent.toFixed(
    1
  )}% acima de 100%)
